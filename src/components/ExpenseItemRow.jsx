...
function ExpenseItemRow({ expense, monthlyAmount, onEdit, onDelete, displayAmount }) {
  const [confirmDelete, setConfirmDelete] = useState(false);
  const shown = displayAmount ?? expense.amount;
  const shownMonthly = displayAmount != null
    ...
    <div className={s.row}>
      <div className={s.main}>
        <div>
          ...
          {expense.assignedTo === "Shared" && expense.splitPercent != null && expense.splitPercent !== 50 && (
            <span className={g.badgeSplit}>{expense.splitPercent}/{100 - expense.splitPercent}</span>
          )}
        </div>
