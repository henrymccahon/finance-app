...
function CategorySummary({
  ...
  const byCategory = useMemo(() => {
    ...
    for (const e of expenses) {
      const base =
        filterPerson && filterPerson !== "Shared" && e.assignedTo === "Shared"
          ? getPersonShare(e, filterPerson)
          : e.amount;
      const monthly = base * (FREQUENCY_MULTIPLIERS[e.frequency] || 1);
      map[e.category] = (map[e.category] || 0) + monthly;
    }
    return Object.entries(map).sort((a, b) => b[1] - a[1]);
  }, [expenses, filterPerson, getPersonShare]);
  ...
  const byPerson = useMemo(() => {
    ...
    for (const e of expenses) {
      if (e.assignedTo === "Shared") {
        ...
        const monthly = e.amount * (FREQUENCY_MULTIPLIERS[e.frequency] || 1);
        ...
        <tbody>
          {byCategory.map(([cat, amt]) => (
            <tr key={cat}>
              ...
              <td className={g.tdRight}>${amt.toFixed(2)}</td>
